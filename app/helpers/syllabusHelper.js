...
function parseModules(modules, faculty, year, type, level) {
  const parsed = {};
  ...
  modules.syllabus.assignments.forEach((element) => {
    ...
    parsed[mod.name] = {
      ...
      price: prices[mod.name] || prices[year].default || prices[year][faculty][level],
    };
  });
  return parsed;
}
...
async function getModulesFromSyllabus(faculty, year, slug, type, level, field) {
  try {
    const newUrl = getProgramModulesUrl(faculty, year, slug);
    const response = await fetchJsonFromSyllabus(newUrl);
    const modules = parseModules(response, faculty, year, type, level, field);
    return { modules };
  } catch (err) {
    console.error(err);
  }
